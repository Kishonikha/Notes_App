...
<pre id="top">ENS/Level 2/Sem 2</pre>
    ...
    if(in_array($fileActualExt,$allowed)){
        if($fileError===0){
            if($fileSize<12*1024*1024){
              ...
              // $fileDestination='PHP/uploads/ens/y2s2/'.$fileNameNew;
                //move_uploaded_file($fileTempname,$fileDestination);
               $fileDestination='PHP/uploads/ens/y2s2/'.$fileName;//uploading with axixting filename
               move_uploaded_file($fileTempname, $fileDestination);

                   ////////////////insert file data into database- database name filesdatasel2s2(filename,filetype,filesize,fileowner)//////////////////////////////
                    $fileowner=$_SESSION['semail'];
                    $query="insert into filesdatasel2s2 values('$fileName','$fileType','$fileSize','$fileowner')";
                    mysqli_query($connect,$query);

                    //////////////////////////////////////////////////////////////////////

                //header("Location:ensl2s2.php?uploadsuccess");
            }else{
...
if(isset($_GET['submitstr'])){
    ...
    $query1="select * from filesdatasel2s2 where Filename like '%$searchtxt%' ";
...
foreach ($result1 as $key => $row1) {
   ...
   // echo "<td><a href='PHP/uploads/ens/y2s2/$k' >Download</a></td>";
...
$query2="select * from filesdatasel2s2;";
