...
$check=$_SESSION['semail'][4];//for give variable to security level script is for security level that no faculty students can go to other faculty notes
...
<pre id="top">AMC/Level 1/Sem 1</pre>
    ...
    if(in_array($fileActualExt,$allowed)){
        if($fileError===0){
            if($fileSize<12*1024*1024){
              ...
              // $fileDestination='PHP/uploads/amc/y1s1/'.$fileNameNew;
                //move_uploaded_file($fileTempname,$fileDestination);
               $fileDestination='PHP/uploads/amc/y1s1/'.$fileName;//uploading with axixting filename
               move_uploaded_file($fileTempname, $fileDestination);

                   ////////////////insert file data into database- database name filesdatas(filename,filetype,filesize,fileowner)//////////////////////////////
                    $fileowner=$_SESSION['semail'];
                    $query="insert into filesdatas values('$fileName','$fileType','$fileSize','$fileowner')";
                    mysqli_query($connect,$query);

                    //////////////////////////////////////////////////////////////////////

                //header("Location:amcl1s1.php?uploadsuccess");
            }else{
...
if(isset($_GET['submitstr'])){
    ...
    $query1="select * from filesdatas where Filename like '%$searchtxt%' ";
...
foreach ($result1 as $key => $row1) {
   ...
   // echo "<td><a href='PHP/uploads/amc/y1s1/$k' >Download</a></td>";
