...
<pre id="top">AMC/Level 3/Sem 1</pre>
    ...
    if(in_array($fileActualExt,$allowed)){
        if($fileError===0){
            if($fileSize<12*1024*1024){
              ...
              // $fileDestination='PHP/uploads/amc/y3s1/'.$fileNameNew;
                //move_uploaded_file($fileTempname,$fileDestination);
               $fileDestination='PHP/uploads/amc/y3s1/'.$fileName;//uploading with axixting filename
               move_uploaded_file($fileTempname, $fileDestination);

                   ////////////////insert file data into database- database name filesdatasal3s1(filename,filetype,filesize,fileowner)//////////////////////////////
                    $fileowner=$_SESSION['semail'];
                    $query="insert into filesdatasal3s1 values('$fileName','$fileType','$fileSize','$fileowner')";
                    mysqli_query($connect,$query);

                    //////////////////////////////////////////////////////////////////////

               // header("Location:amcl3s1.php?uploadsuccess");
            }else{
...
if(isset($_GET['submitstr'])){
    ...
    $query1="select * from filesdatasal3s1 where Filename like '%$searchtxt%' ";
...
foreach ($result1 as $key => $row1) {
   ...
   // echo "<td><a href='PHP/uploads/amc/y3s1/$k' >Download</a></td>";
...
$query2="select * from filesdatasal3s1;";
